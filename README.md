<div align="center">

# Pushpendra Solanki

**Backend & AI Engineer**

<img
  src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=17&duration=2600&pause=900&color=58A6FF&center=true&vCenter=true&width=650&lines=Backend+Engineering+%C2%B7+AI+Systems;Retrieval+%C2%B7+Memory+%C2%B7+Evaluation;Benchmarking+%C2%B7+System+Reliability;Build+%C2%B7+Measure+%C2%B7+Understand+%C2%B7+Improve"
  alt="Typing SVG"
/>

<a href="https://www.linkedin.com/in/pushpendra-solanki-557600335/">
  <img
    src="https://img.shields.io/badge/LinkedIn-161B22?style=flat-square&logo=linkedin&logoColor=58A6FF"
    alt="LinkedIn"
  />
</a>
&nbsp;
<a href="mailto:pushpendrasolanki022@gmail.com">
  <img
    src="https://img.shields.io/badge/Email-161B22?style=flat-square&logo=gmail&logoColor=EA4335"
    alt="Email"
  />
</a>

</div>

---

## About

Backend & AI Engineer working across **backend systems, retrieval, AI memory and evaluation**.

I turn system behavior into **measurable experiments** — designing benchmarks, investigating failure modes and improving systems based on evidence.

> **Build → Measure → Understand → Improve**

---

## Current Focus

| Area | Focus |
|---|---|
| **AI Memory & Retrieval** | Long-term memory and retrieval systems |
| **Evaluation** | Benchmarks and reproducible experiments |
| **Python & Systems Engineering** | Improving Python backend and systems engineering skills |
| **Backend Engineering** | Reliable and scalable backend services |
| **System Reliability** | Measurement, debugging and failure analysis |
| **AI Infrastructure** | Agents, retrieval and long-term memory systems |

---

## Tech Stack

<div align="center">

<img
  src="https://skillicons.dev/icons?i=python,java,spring,postgres,mysql,docker,linux,git,github&theme=dark"
  alt="Tech Stack"
/>

<br>

`Python` · `Java` · `Spring Boot` · `REST APIs` · `JPA / Hibernate`

`PostgreSQL` · `MySQL` · `Docker` · `Linux` · `Git`

`Retrieval` · `AI Memory` · `Evaluation` · `Benchmarking` · `LLMs`

</div>

---

## GitHub Activity

<div align="center">

<img
  src="https://ghchart.rshah.org/58A6FF/Pushpendra022"
  width="100%"
  alt="Pushpendra Solanki's GitHub Contribution Chart"
/>

<sub>Live GitHub contribution activity</sub>

</div>

---

<div align="center">

**Measure first. Understand the failure. Then improve the system.**

<sub>Backend Engineering · AI Memory · Retrieval · Evaluation · System Reliability</sub>

</div>
